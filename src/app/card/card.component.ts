...
export class CardComponent implements OnInit {
  ...
  trackProgress(learned: boolean) {
    console.log(learned ? 'trackProgress: learned' : 'trackProgress: need to train');
    this.progressService.addProgress(this.currentBibleWord, learned);
    this.updateCurrentBibleWord();
  }

  updateCurrentBibleWord() {
    console.log('updateCurrentBibleWord');
  ...
  updateLearnedStatus() {
    ...
    console.log(this.currentBibleWord);
  ...
  get passage(): string {
    ...
    return `${bible.shortBook} ${bible.chapter}:${bible.verse}`;
  }
}
